...
template<typename T>
inline bool
IsFloatTheSameAs(T e, T def)
{
  return (e == def) || ((def != def) && (e != e));
}
template<>
inline bool
IsTheSameAs<float>(float e, float def)
{
  return IsFloatTheSameAs(e, def);
}
template<>
inline bool
IsTheSameAs<double>(double e, double def)
{
  return IsFloatTheSameAs(e, def);
}
#endif
...
template<typename T>
inline bool
IsOutRange(const T& v, const T& low, const T& high)
{
  return (v < low) || (high < v);
}
...
template<typename T>
inline bool
IsInRange(const T& v, const T& low, const T& high)
{
...
protected:
  void MutateImpl(flatbuffers::integral_constant<bool, true>,
                  uoffset_t i,
                  const T& val)
  {
    FLATBUFFERS_ASSERT(i < size());
  ...
  void MutateImpl(flatbuffers::integral_constant<bool, false>,
                  uoffset_t i,
                  const T& val)
  {
...
static inline bool
StringLessThan(const char* a_data,
               uoffset_t a_size,
               const char* b_data,
               uoffset_t b_size)
{
...
inline uint8_t*
Allocate(Allocator* allocator, size_t size)
{
...
inline void
Deallocate(Allocator* allocator, uint8_t* p, size_t size)
{
  if (allocator)
...
inline uint8_t*
ReallocateDownward(Allocator* allocator,
                   uint8_t* old_p,
                   size_t old_size,
                   size_t new_size,
                   size_t in_use_back,
...
const T*
data(const std::vector<T, Alloc>& v)
{
...
inline bool
BufferHasIdentifier(const void* buf,
...
// clang-format off
  ...
  {
    size_t end;
    return !str || (VerifyVectorOrString(
  ...
  bool VerifyVectorOrString(const uint8_t* vec,
                            size_t elem_size,
...
template<typename T>
void
FlatBufferBuilder::Required(Offset<T> table, voffset_t field)
{
...
inline uoffset_t
GetPrefixedSize(const uint8_t* buf)
{
...
template<typename T>
bool
IsFieldPresent(const T* table, typename T::FlatBuffersVTableOffset field)
{
